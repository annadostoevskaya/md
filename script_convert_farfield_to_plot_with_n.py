...
- Each data row must contain at least: theta phi value (in dB)
Output:
- For each input file, creates <input_name>.raw.txt next to it.
- Values are normalized so that the maximum = 0 dB (i.e. max is subtracted).
...
def parse_farfield_file(input_file: str, output_file: str) -> None:
    ...
    all_vals = [v for phi_dict in data.values() for v in phi_dict.values()
                if not math.isnan(v)]
    if all_vals:
        ...
        for phi in phi_list:
            for theta in theta_list:
                if theta in data[phi]:
                    normalized = (data[phi][theta] - min_val) / val_range
    ...
    print("Raw range  : [{:.6e}, {:.6e}]  →  normalized to [0, 1]".format(min_val, max_val))
    print("-" * 60)
